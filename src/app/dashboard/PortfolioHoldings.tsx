...
import {
  PORTFOLIO_HOLDINGS,
  getStockDataForHolding,
} from "@/lib/mock-portfolio-holdings";
...
export default function PortfolioHoldings() {
  const portfolioSummary = useMemo(() => {
    ...
    const holdingsWithCurrentData = PORTFOLIO_HOLDINGS.map((holding) => {
      ...
      const dailyChange =
        (currentStockData.price - holding.averageCost) * holding.quantity; // Simplified daily change for mock
    ...
    const totalProfitOrLossPercentage =
      totalCost > 0 ? (totalProfitOrLoss / totalCost) * 100 : 0;
    ...
    <div className={styles.portfolioHoldingsContainer}>
      ...
      <div className={styles.summaryCards}>
        ...
        <div className={styles.summaryCard}>
          ...
          <p
            className={`${styles.summaryValue} ${
              portfolioSummary.totalProfitOrLoss >= 0
                ? styles.positive
                : styles.negative
            }`}
          >
            {formatCurrency(portfolioSummary.totalProfitOrLoss)} (
            {formatPercentage(portfolioSummary.totalProfitOrLossPercentage)})
          </p>
        </div>
      </div>
